...
impl EdwardsPoint {
    ...
    /// Compute \\(aA + bB\\) in variable time, where \\(B\\) is the Ed25519 basepoint.
    pub fn vartime_double_scalar_mul_basepoint_timed(
        a: &Scalar,
        A: &EdwardsPoint,
        b: &Scalar,
    ) -> (EdwardsPoint, Duration, Duration, Duration) {
        scalar_mul::vartime_double_base::mul_timed(a, A, b)
    }

    /// Compute \\(aA + bB\\) in variable time, where \\(B\\) is the Ed25519 basepoint.
    pub fn vartime_double_scalar_mul_basepoint_byz_score(
        a: &Scalar,
        A: &EdwardsPoint,
        b: &Scalar,
    ) -> (EdwardsPoint, usize) {
        scalar_mul::vartime_double_base::mul_byz_score(a, A, b)
    }
}
